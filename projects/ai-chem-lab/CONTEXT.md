...
## 当前阶段
**架构决策定稿期** — 物理实验室已就绪，2026-07-24 完成技术架构重大调整：
- ~~LabVLA 端到端路线~~ → **分层架构（LLM + Specialist + Classical Control）**
- ~~训练框架~~ → **Diffusion Policy（BC + RL 三阶段训练）**
- ~~触觉融合~~ → **12×12×3 CNN编码 → 条件信号喂入 Diffusion Policy**
- 宇树 G1-D 评估完成，问题清单已准备

## 2026-07-24 🎯 重大架构决策：端到端 VLA → 分层架构

### 核心决策
- **放弃** LabVLA 端到端路线（Qwen3-VL-4B + DiT → 一个模型包揽全部）
- **转向** 三层分层架构：LLM（大脑）+ Specialist（小脑）+ Classical Control（脊髓）

### 新架构概览

```
🧠 Layer 1 — 大脑 (Frontier LLM，云端API)
    GPT-5/Claude-4 → SOP理解 / 24步任务规划 / 安全监控 / 科研推理
    频率 0.1-1 Hz，延迟 2-5s（完全可接受，不参与实时控制）

⚡ Layer 2 — 小脑 (Specialist，本地服务器 GPU)
    ① 场景感知：SAM2 + DINOv2 → 物体检测+3D位姿（微调，非从头训练）
    ② 技能策略：Diffusion Policy → 条件生成关节轨迹（核心训练对象）
    ③ 技能选择：轻量规则 + MLP
    频率 10-100 Hz，延迟 50-110ms

🦾 Layer 3 — 脊髓 (Classical Control，机器人本地)
    视觉伺服 AprilTag (±0.05mm) / 力控闭环 1000Hz / IK求解器
    不经过网络，频率 200-1000Hz
```

### 触觉融合
- 触觉不单独建模型 → 作为条件信号喂入 Diffusion Policy
- 原始 124 点 → 12×12×3 编码（压力+变化率+振动）→ 4层CNN → 64维特征向量
- 与图像特征(256dim) + 关节角(16dim) + 指令(16dim) 拼接为 352dim 条件向量

### 训练策略（三阶段）
1. **BC (行为克隆)**: 人手示教 500-2000次/技能 → 模型学会"像人那样做"
2. **RL (强化学习)**: MuJoCo 仿真 10万-100万步 → 优化平滑度/成功率
3. **真机微调**: 50-200次 → 弥合 sim-to-real gap

### 部署方案
- ☁️ **云端**: LLM API 按量付费（免部署）
- 🏢 **本地服务器**: 场景感知(GPU) + 技能策略(GPU) + 数据记录
- 🤖 **Jetson Orin**: Diffusion Policy 本地推理(10-30ms) + AprilTag + ROS2网关
- 🔧 **MCU**: 力控闭环 1000Hz（不经过网络）

### 宇树 G1-D 评估
- 推荐型号：**旗舰版C (U8) — ¥311,000**（Dex3-1 力控三指有触觉 + 轮式底盘，33自由度）
- 关键待确认点：底层 SDK 开放/ROS支持/控制频率/触觉数据回读/是否可绕过数采软件
- 问题清单已准备（见 `hardware/unitree-G1D-问题清单-20260724.md`）

### 新沉淀文档
- `hardware/unitree-G1D-问题清单-20260724.md` — 宇树技术问询清单
- `architecture/training-architecture.md` — 训练架构详解（BC→RL→真机微调）
- `josan-vault/01-AI化学实验室/技术方案/技术架构总览.md` — 完整技术架构文档
